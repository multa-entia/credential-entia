...
class GetBridgeByUserIdAndRightIdManagerCommandTest {
    ...
    void shouldCheckCommandExecution_ifServiceIsAbsence() {
        ...
        new GetBridgeByUserIdAndRightIdManagerCommand(consumer, new ObjectId(), new ObjectId()).execute();

        assertThat(okHolder.get()).isFalse();
        assertThat(codeHolder.get()).isEqualTo(CR.get(GetBridgeByUserIdAndRightIdManagerCommand.Code.SERVICE_IS_ABSENCE));
    }
    ...
    void shouldCheckCommandExecution_ifEntityIsAbsence() {
        ...
        Supplier<BridgeService> bridgeServiceSupplier = () -> {
            ...
            Mockito
                    .when(service.getByUserAndRightIds(Mockito.any(), Mockito.any()))
                    .thenReturn(result);

            return service;
        };

        GetBridgeByUserIdAndRightIdManagerCommand command
                = new GetBridgeByUserIdAndRightIdManagerCommand(consumer, new ObjectId(), new ObjectId());
    ...
    void shouldCheckCommandExecution() {
        ...
        Consumer<Result<ManagerDatum>> consumer = result -> {
            okHolder.set(result.ok());
            codeHolder.set(result.value().get(GetBridgeByUserIdAndRightIdManagerCommand.PROPERTY_BRIDGES).value());
        };
