...
class GetUsrBySurnameManagerCommandTest {
    ...
    void shouldCheckCommandExecution_ifServiceIsAbsence() {
        ...
        new GetUsrBySurnameManagerCommand(consumer, Faker.str_().random()).execute();

        assertThat(okHolder.get()).isFalse();
        assertThat(codeHolder.get()).isEqualTo(CR.get(GetUsrBySurnameManagerCommand.Code.SERVICE_IS_ABSENCE));
    }
    ...
    void shouldCheckFindingBySurname_ifFail() {
        ...
        GetUsrBySurnameManagerCommand command = new GetUsrBySurnameManagerCommand(consumer, Faker.str_().random());
